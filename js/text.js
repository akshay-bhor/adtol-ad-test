  ...
  fetchAd(token)
    .then((res) => {
      ...
      for (let i in res) {
        ...
        html += `
            <div style='background-color:#fff;border-radius:2px;margin:.5rem;padding:.5rem;
            box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);transition:all 0.3s cubic-bezier(.25,.8,.25,1)'>
            <div>
            <a target='_blank' style='margin:2px;padding:2px;text-align:left!important;text-decoration:none'${rel} href='${res[i].process}'>
            <b>${res[i].title}</b></a>
            </div>
            <div style='color:#333;text-align:left!important;'>
            <a target='_blank' style='margin:2px;padding:2px;text-align:left!important;display:inline-block' href='https://www.adtol.com'>
            <span style='background:green;display:inline-block;padding:0 2px 0 2px;border-radius:4px;color:#fff;font-size:14px;'>Ad</span>
            </a> ${res[i].domain}</div>
            <div style='margin:0px;padding:2px;color:#777;text-align:left!important'>${res[i].desc}</div></div>
        `;
      }
